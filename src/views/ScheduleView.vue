...
interface Program { time: string; title: string; category: string; duration: string; isLive: boolean }
...
const JUTRO_DATA: ChannelData[] = [
  {
    name: 'iTVT', icon: 'iT',
    programs: [
      { time: '08:00', title: 'Poranek Technologiczny', category: 'Informacje', duration: '60', isLive: false },
      { time: '10:00', title: 'Fast News IT', category: 'Informacje', duration: '30', isLive: false },
      { time: '14:00', title: 'GStreamer Deep Dive', category: 'Edukacja', duration: '60', isLive: false },
      { time: '19:00', title: 'Wydarzenia Dnia', category: 'Informacje', duration: '60', isLive: true },
  ...
  },
  {
    ...
    programs: [
      { time: '09:00', title: 'Poranny Stream', category: 'Rozrywka', duration: '90', isLive: false },
      { time: '14:00', title: 'Speedrun Session', category: 'Gry', duration: '120', isLive: false },
...
function setDay(day: 'dzis' | 'jutro') {
  activeDay.value = day
...
function isHighlighted(title: string): boolean {
  return searchHighlight.value.length > 0 &&
    title.toLowerCase().includes(searchHighlight.value.toLowerCase())
}
</script>
...
<template>
  <div class="schedule">
    ...
    <div class="schedule-grid">
      <div v-for="ch in filteredData" :key="ch.name + activeDay" class="channel-card">
        ...
        <div class="program-list">
          <div v-for="(prog, i) in ch.programs" :key="i"
            ...
            <div class="program-meta">{{ prog.category }} &middot; {{ prog.duration }} min</div>
...
.filter-btn {
  ...
  padding: 6px 14px; font-size: 13px; transition: all 0.2s;
}
...
.program-card {
  ...
  cursor: pointer; transition: filter 0.2s, border-color 0.2s, background 0.2s;
}
