...
onMounted(async () => {
  allItems.value = await fetchChannelVideos()
  loading.value = false
})
...
<template>
  <div class="vod-library">
        ...
        <div class="btn-group">
          <div class="dropdown-wrapper">
            ...
            <div v-if="showFilters" class="dropdown-panel" @click.stop>
              <div class="dropdown-item">
                <input v-model="searchTerm" class="dropdown-input" placeholder="Szukaj w VOD..." />
              </div>
          ...
          <div class="dropdown-wrapper">
            ...
            <div v-if="showSort" class="dropdown-panel right" @click.stop>
              <div class="dropdown-option" :class="{ checked: activeSort === 'date' }" @click="setSort('date')">
                <span class="check" />
                Data (najnowsze)
              </div>
              <div class="dropdown-option" :class="{ checked: activeSort === 'title' }" @click="setSort('title')">
                <span class="check" />
                Tytuł (A-Z)
              </div>
            </div>
    ...
    <div v-if="loading" class="loading-state">
      <span class="loading-text">Ładowanie biblioteki VOD...</span>
    </div>

    <div v-else-if="items.length === 0" class="loading-state">
      <span class="loading-text">Brak wyników</span>
    </div>
